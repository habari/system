...
<?php // Fetch user information
  ...
  // The current user is $user.
	// The user being edited is $edit_user, which is retrieved via from the username specified on the URL
...
<form name="update-profile" id="update-profile" action="<?php URL::out('admin', 'page=user'); ?>" method="post">
	<div class="container settings user userinformation" id="userinformation">
		...
		<h2><?php echo $possessive; ?></h2>
			...
			<div class="item clear" id="displayname">
				...
				<span class="pct80">
					<input type="text" name="displayname" class="border big" value="<?php echo $edit_user->info->displayname; ?>">
				</span>
			</div>
			...
			<span class="pct80">
					<input type="text" name="username" class="border" value="<?php echo $edit_user->username; ?>">
				</span>
			</div>	
			...
			<span class="pct80">
					<input type="text" name="email" class="border" value="<?php echo $edit_user->email; ?>">
				</span>
			</div>
			...
			<span class="pct80">
				<input type="text" name="imageurl" class="border" value="<?php echo $edit_user->info->imageurl; ?>">
				</span>
			</div>
	</div>
	...
	<div class="container settings regionalsettings" id="regionalsettings">
		...
		<div class="item clear" id="timezone">
			...
			<span class="pct20">
				...
				<?php foreach (DateTimeZone::listIdentifiers() as $tz_identifier) : ?>
					<option value="<?php echo $tz_identifier; ?>" <?php echo ( $edit_user->info->locale_tz == $tz_identifier) ? 'selected="selected"' : '' ?>><?php echo $tz_identifier; ?></option>
				<?php endforeach; ?>
				</select>
			</span>
		</div>
		...
		<div class="item clear" id="date_format">
			...
			<span class="pct20">
				<input type="text" name="locale_date_format" class="border" value="<?php echo $edit_user->info->locale_date_format ?>">
			</span>
			<span class="pct50 helptext">
				...
				<span><?php HabariDateTime::date_create()->out($edit_user->info->locale_date_format) ?></span>
			</span>
		</div>
		...
		<div class="item clear" id="time_format">
			...
			<span class="pct20">
				<input type="text" name="locale_time_format" class="border" value="<?php echo $edit_user->info->locale_time_format ?>">
			</span>
			<span class="pct50 helptext">
				<span>Use a date format string usable by the php date() function. See <a href="http://php.net/date">php.net/date</a> for details</span>
				<span><?php HabariDateTime::date_create()->out($edit_user->info->locale_time_format) ?></span>
			</span>
		</div>
	</div>

	<?php Plugins::act( 'theme_admin_user', $edit_user ); ?>
...
<?php
if ( Posts::count_by_author( $edit_user->id, Post::status('published') ) ) {
	echo $possessive . _t(' five most recent published posts:') . "<br>\n";
	$posts = Posts::get( array( 'user_id' => $edit_user->id, 'limit' => 5, 'status' => Post::status('published'), ) );
	if ( count( $posts ) > 0 ) {
		echo "<ul>\n";
...
if ( $user != $edit_user ) {
	...
	echo '<p><input type="hidden" name="user_id" value="' . $edit_user->id . '"><p>'."\n";
	echo '<p><ul><li><input type="radio" name="reassign" id="purge" value="0" checked>' . _t('Delete posts') . '</li>';
	$author_list = DB::get_results('SELECT id,username FROM ' . DB::table('users') . ' WHERE username <> ? ORDER BY username ASC', array( $edit_user->username) );
