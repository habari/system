...
class RewriteRules extends \ArrayObject
{
	...
	public static function add_system_rules( $rules )
	{
		$default_rules = array(
			...
			array( 'name' => 'admin_ajax_facets', 'parse_regex' => '#^admin_ajax/(?P<context>facets)/(?P<component>facets|values)/?$#i', 'build_str' => 'admin_ajax/{$context}/{$component}', 'handler' => 'AdminPostsHandler', 'action' => 'admin_ajax', 'priority' => 8, 'description' => 'Authenticated ajax handling for retrieving post search facets' ),
			array( 'name' => 'admin_ajax_tag_facets', 'parse_regex' => '#^admin_ajax/(?P<context>tag_facets)/(?P<component>facets|values)/?$#i', 'build_str' => 'admin_ajax/{$context}/{$component}', 'handler' => 'AdminTagsHandler', 'action' => 'admin_ajax', 'priority' => 8, 'description' => 'Authenticated ajax handling for retrieving post search facets' ),
