...
class RewriteRules extends ArrayObject {
	...
	static public function add_system_rules($rules)
	{
		$default_rules= array(
			...
			array( 'name' => 'display_feed_by_type', 'parse_regex' => '%^feed/(?P<feed_type>atom|rs[sd])/?$%i', 'build_str' => 'feed/{$feed_type}', 'handler' => 'FeedHandler', 'action' => 'display_feed', 'priority' => 5, 'description' => 'Return feed per specified feed type' ),
			...
			array( 'name' => 'collection', 'parse_regex' => '%^atom/(?P<index>.+)/?$%i', 'build_str' => 'atom/{$index}', 'handler' => 'AtomHandler', 'action' => 'collection', 'priority' => 8, 'description' => 'Atom collection' ),
			...
			array( 'name' => 'entry_comments', 'parse_regex' => '%^(?P<slug>[^/]+)/atom/comments/?$%i', 'build_str' => '{$slug}/atom/comments', 'handler' => 'AtomHandler', 'action' => 'entry_comments', 'priority' => 8, 'description' => 'Entry comments' ),
			array( 'name' => 'comments', 'parse_regex' => '%^atom/comments/?$%i', 'build_str' => 'atom/comments', 'handler' => 'AtomHandler', 'action' => 'comments', 'priority' => 7, 'description' => 'Entries comments' ),
			array( 'name' => 'tag_collection', 'parse_regex' => '%^tag/(?P<tag>[^/]+)/atom/?$%i', 'build_str' => 'tag/{$tag}/atom', 'handler' => 'AtomHandler', 'action' => 'tag_collection', 'priority' => 8, 'description' => 'Atom Tag Collection' ),
