...
// Debugging function to dump a single dom node with a bunch of information about it.
	function dump_node($echo = true)
	{
		...
		$string .= " HTMLDom::HDOM_INNER_INFO: '";
		if ( isset($node->_[HTMLDom::HDOM_INFO_INNER]) ) {
			$string .= $node->_[HTMLDom::HDOM_INFO_INNER] . "'";
		}
		else {
			$string .= ' NULL ';
		}
...
// get dom node's inner html
	function innertext()
	{
		...
		if ( isset($this->_[HTMLDom::HDOM_INFO_TEXT]) ) {
			return $this->dom->restore_noise($this->_[HTMLDom::HDOM_INFO_TEXT]);
		}
...
// render inner text
		if ( isset($this->_[HTMLDom::HDOM_INFO_INNER]) ) {
// If it's a br tag...  don't return the HTMLDom::HDOM_INNER_INFO that we may or may not have added.
			if ( $this->tag != "br" ) {
...
//no value attr: nowrap, checked selected...
			if ( $val === true ) {
				...
				$ret .= $key . $this->_[HTMLDom::HDOM_INFO_SPACE][$i][1] . '=' . $this->_[HTMLDom::HDOM_INFO_SPACE][$i][2] . $quote . $val . $quote;
			}
		}
...
// used to be: if (($levle=count($selectors[0]))===0) return array();
			...
			$head = array($this->_[HTMLDom::HDOM_INFO_BEGIN] => 1);
...
//elements that do NOT have the specified attribute
	...
	function __set($name, $value)
	{
		...
		if ( !isset($this->attr[$name]) ) {
			$this->_[HTMLDom::HDOM_INFO_SPACE][] = array(' ', '', '');
